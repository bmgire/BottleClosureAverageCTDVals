...
#################################################
# This program will take derived CTD data that has been processed from SBE software per CalCOFI Instructions. 
# The derived data will already need to be to ASCII in a .asc file (again per the CalCOFI instructions). 
# You will also need to have the .bl file for the cast. 
# It will average data from bottle closure to 4 seconds (user editable) before the bottle closure.
# It uses the .bl file to get the bottle closure scan number. 
...
# The new averaged CTD can be compared to compare with the bottle chemistry values.    
...
def createAverages():
    ...
    for btl in range(bottlesRange):
        ...
        mean = allScansData.iloc[:, 2:].mean(axis=0)
    ...
    # Once out of the loop, write result to file. 
